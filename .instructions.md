...
Successfully healed selectors are cached in `ai-reports/healing-cache.json` for reuse across runs:
- **TTL:** 7 days (entries expire automatically)
- **Confidence gate:** Only heals with confidence ≥ 0.9 are written to cache
- **Trust threshold:** Cache entries need `successCount ≥ 2` before being used (prevents bad heals from poisoning future runs)
- **Auto-invalidation:** If a cached selector fails on retry, it's immediately removed
- **Manual reset:** Set `ai-reports/healing-cache.json` to `{}` to force fresh healing
...
3. Step 1: Check healing cache (if trusted entry exists, use it directly)
   - If cached selector also fails → invalidate cache, continue to live heal
4. Step 2: Try dismissing blocking popups
   - If popup dismissed and retry succeeds → done
5. Step 3: Invoke RuntimeHealingGraph
   a. CSS Strategy:
...
3. For each step: extracts LIVE DOM snapshot → GPT picks the real selector → Playwright executes it
4. Records all actions + working selectors
5. Generates page objects and test specs from the recording

**Key principle:** GPT never invents selectors. It reads the actual DOM via `page.evaluate()` and picks from elements that exist on the page. This eliminates hallucination.

### How the DOM-Based Selector Resolution Works

```
Step: "click the sidebar navigation button with text Cameras"
  ↓
extractDOMSnapshot(page) → page.evaluate() extracts ALL buttons, links, inputs, headings
  ↓
DOM sent to GPT: <button role="button" data-nav="true">Cameras</button>
  ↓
GPT resolves: page.getByRole('button', { name: 'Cameras' }).first()
  ↓
Playwright executes → SUCCESS → selector recorded
```
...
| Suite (smoke) | `npm run ai:author:smoke` | Run smoke .md suite with login |
| Suite (custom) | `npm run ai:author:suite -- --suite path.md` | Batch execution |
...
**Entry:** Precondition description
**Exit:** Expected postcondition
...
**Suite header options** (apply to all TCs unless overridden):
| Option | Purpose |
|--------|---------|
| `login: true` | Auto-login before each TC |
| `locators: getByRole, getByText` | Preferred Playwright locator strategies |
| `output: tests/generated/smoke` | Where to write generated spec files |
| `pages: framework/pages/generated/smoke` | Where to write generated page objects |
| `tags: @smoke @tc` | Tags added to describe() block |

### Output Artifacts

All outputs go to `ai-reports/nl-authoring/`:
- `action-log-{timestamp}.txt` — Human-readable execution log
- `suite-{name}-{timestamp}.json` — Structured results with pass/fail per TC
- `suite-{name}-{timestamp}.txt` — Human-readable suite summary
- Generated spec → `tests/generated/smoke/` (per output config)
- Generated POs → `framework/pages/generated/smoke/` (per pages config)
...
| `ai-reports/healing-cache.json` | Cached healed selectors (reused across runs) | Per successful heal |
...
### Execute (Demo-Ready Pipeline)

| Script | Description |
|--------|-------------|
| `npm run execute:smoke` | Run existing smoke spec → Allure report (no generation, fast) |
| `npm run execute:smoke:heal` | Same as above with AI healing enabled |
| `npm run execute:smoke:gen` | **Full pipeline:** .md → real browser authoring → generate spec → run → Allure report |
| `npm run execute:smoke:gen:heal` | Full pipeline with AI healing enabled during execution |
| `npm run execute:report` | Open existing Allure report |

#### Full Pipeline Flow (`execute:smoke:gen`)

```
┌─────────────────────────────────────────────────────────────────────────┐
│  smoke.md (plain English test cases)                                     │
│                                                                          │
│  Step 1: npm run ai:author:smoke                                         │
│    → Parses .md into structured steps (GPT)                              │
│    → Opens real browser, logs in                                         │
│    → For EACH step: extracts live DOM → GPT picks real selector → executes│
│    → Records all working selectors + actions                             │
│    → Generates .spec.js + PageObject.js from recording                   │
│    → Writes to tests/generated/smoke/ + framework/pages/generated/smoke/ │
│                                                                          │
│  Step 2: npx playwright test tests/generated/smoke/ --project chrome     │
│    → Executes the generated spec (verified locators from real DOM)        │
│                                                                          │
│  Step 3: npm run report:allure                                           │
│    → Generates and opens the Allure report                               │
└─────────────────────────────────────────────────────────────────────────┘
```

**Why this avoids hallucination:**
- GPT never invents selectors — it picks from the actual DOM snapshot extracted via `page.evaluate()`
- Every selector is validated by executing it in the real browser before recording
- The generated spec only contains selectors that WORKED during the authoring run
- If the page changes, re-run `execute:smoke:gen` to regenerate from the new DOM

**When to use which:**
| Scenario | Command |
|----------|---------|
| Quick re-run (spec already exists and works) | `npm run execute:smoke` |
| UI changed or new test cases added to .md | `npm run execute:smoke:gen` |
| Broken locators need healing at runtime | `npm run execute:smoke:heal` |
| Full regeneration + healing safety net | `npm run execute:smoke:gen:heal` |
...
| `npm run ci:test:smoke` | CI: run smoke suite (chrome project) |
...
### Issue: Healing cache returns wrong selector / heals to wrong element

**Cause:** A previous healing attempt found a selector that passed click validation (action didn't throw) but navigated to the wrong page. That wrong heal got cached and reused on every subsequent run.

**Fix:**
1. Clear the cache: delete contents of `ai-reports/healing-cache.json` (set to `{}`)
2. Re-run with healing enabled — GPT will attempt fresh healing

**Prevention (built-in safeguards):**
- Only heals with **confidence ≥ 0.9** are persisted to cache
- Cache entries require **MIN_SUCCESS_COUNT = 2** validations before being trusted
- If a cached selector throws on retry, it's automatically invalidated
- First-time heals always go through live GPT (cache never short-circuits on first success)

**File:** `framework/ai/storage/healing-cache.js`
